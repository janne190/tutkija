    ...
    *,
    ...
    if gold_labels is not None and not gold_labels.empty:
        ...
        if seed_indices:
            ...
            fallback = "default_prob_0.5"
    # Jos todennäköisyydet eivät vaihtele (esim. fallback 0.5), käytä
...
class LogisticRegressionPipeline:
    ...
    def __init__(
        self, *, vectorizer: TfidfVectorizer, classifier: LogisticRegression
    ) -> None:
...
def _build_stats(
    ...
    *,
    ...
    excluded_model = (
        int(((labels == EXCLUDED) & (reasons_len == 0)).sum())
        if not labels.empty
        else 0
    )
